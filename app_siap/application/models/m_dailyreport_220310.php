	...
	function getData($params)
	{
		...
		if (!empty($params['v_satkerid'])) {
			$cond_where =
				...
				. $params['v_satkerid'] .
				"' || '%' ORDER BY i.gol , COALESCE(c.satkerid,'99'), (CASE WHEN a.pegawaiid = (SELECT s1.kepalaid FROM satker s1 WHERE s1.satkerid = c.satkerid) THEN '1' ELSE '0' END) DESC, a.waktu";
		} else {
			$cond_where =
				" where g.statusid = '"
				. $params['v_statusid'] .
				...
				"','YYYY/MM/DD') AND TO_DATE('"
				. $params['v_tglselesai'] .
				"','YYYY/MM/DD') ORDER BY i.gol , COALESCE(c.satkerid,'99'), (CASE WHEN a.pegawaiid = (SELECT s1.kepalaid FROM satker s1 WHERE s1.satkerid = c.satkerid) THEN '1' ELSE '0' END) DESC, a.waktu";
		}
